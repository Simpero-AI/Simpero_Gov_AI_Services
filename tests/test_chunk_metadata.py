...
def _char(ch: str, page: int = 1) -> CharBox:
    return CharBox(
        char=ch, x0=0, top=0, x1=1, bottom=1, page=page, is_boilerplate=False, precision="word"
    )
...
def _table(
    cells: list[TableCellRecord], *, scale_context: str | None = None, mult: float | None = None
) -> TableElement:
    return TableElement(
        page=1,
        bbox=None,
...
def test_element_type_labels_prose_table_and_chart() -> None:
    ...
    chart = ChartElement(
        page=1,
        bbox=BBox(x0=0, top=0, x1=10, bottom=10, page=1),
        caption_text="Rev chart",
        surrounding_text="",
...
def test_scale_context_carried_onto_table_chunk() -> None:
    cells = [
        _cell(0, 0, "($ in thousands)", header=True),
        _cell(0, 1, "2005", header=True),
        _cell(1, 0, "Revenue"),
        _cell(1, 1, "15,295"),
    ]
...
def test_scale_context_survives_when_header_is_in_a_different_chunk() -> None:
    ...
    chunks = chunk_table(
        _table(cells, scale_context=None), page, 0, document_id="s", source_file="d.pdf"
    )
...
def test_scale_context_on_every_fragment_of_a_split_table() -> None:
    ...
    page = _page("dummy")
    frags = chunk_table(
        _table(header + body, scale_context="(rooms)"),
        page,
        0,
        document_id="s",
        source_file="d.pdf",
